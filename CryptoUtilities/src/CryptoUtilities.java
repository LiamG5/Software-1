...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        NaturalNumber runner = new NaturalNumber2(m);
        boolean lost = true;
        ...
        while (lost) {
            if (tempn.divide(runner).isZero()
                    && tempm.divide(runner).isZero()) {
                lost = false;
                n.transferFrom(runner);
                m.clear();
            } else {
                runner.decrement();
                tempn.copyFrom(n);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber nM = new NaturalNumber2(n);
        nM.decrement();
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 0; i < testNum; i++) {
                while (inval) {
                    test.transferFrom(randomNumber(n));
                    if (test.compareTo(TWO) > 0 && test.compareTo(nM) < 0) {
                        inval = false;
                    }
                }

                ans = !isWitnessToCompositeness(test, n);
